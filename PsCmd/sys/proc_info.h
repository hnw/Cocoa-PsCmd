...
struct _proc_taskallinfo {
	struct _proc_bsdinfo	pbsd;
	struct _proc_taskinfo	ptinfo;
};

#define MAXTHREADNAMESIZE 64

struct _proc_threadinfo {
...
struct _vnode_info {
	struct _vinfo_stat	vi_stat;
	int			vi_type;
	int			vi_pad;
	fsid_t			vi_fsid;
};

struct _vnode_info_path {
	struct _vnode_info	vip_vi;
	char			vip_path[MAXPATHLEN];	/* tail end of it  */
};

struct _vnode_fdinfo {
	struct _proc_fileinfo	pfi;
	struct _vnode_info	pvi;
};

struct _vnode_fdinfowithpath {
	struct _proc_fileinfo	pfi;
	struct _vnode_info_path	pvip;
};

struct _proc_regionwithpathinfo {
	struct _proc_regioninfo	prp_prinfo;
	struct _vnode_info_path	prp_vip;
};

struct _proc_vnodepathinfo {
	struct _vnode_info_path	pvi_cdir;
	struct _vnode_info_path	pvi_rdir;
};

struct _proc_threadwithpathinfo {
	struct _proc_threadinfo	pt;
	struct _vnode_info_path	pvip;
};
...
struct _tcp_sockinfo {
	struct _in_sockinfo		tcpsi_ini;
	int				tcpsi_state;
...
enum {
	...
	_SOCKINFO_UN		= 3,
	_SOCKINFO_NDRV		= 4,
	_SOCKINFO_KERN_EVENT	= 5,
	_SOCKINFO_KERN_CTL	= 6
};

struct _socket_info {
	struct _vinfo_stat			soi_stat;
	...
	struct _sockbuf_info			soi_rcv;
	struct _sockbuf_info			soi_snd;
	int					soi_kind;
	uint32_t				rfu_1;		/* reserved */
	union {
		struct _in_sockinfo	pri_in;			/* SOCKINFO_IN */
		struct _tcp_sockinfo	pri_tcp;		/* SOCKINFO_TCP */
		struct _un_sockinfo	pri_un;			/* SOCKINFO_UN */
		struct _ndrv_info	pri_ndrv;		/* SOCKINFO_NDRV */
		struct _kern_event_info	pri_kern_event;		/* SOCKINFO_KERN_EVENT */
		struct _kern_ctl_info	pri_kern_ctl;		/* SOCKINFO_KERN_CTL */
	}					soi_proto;
};

struct _socket_fdinfo {
	struct _proc_fileinfo	pfi;
	struct _socket_info	psi;
};



struct _psem_info {
	struct _vinfo_stat	psem_stat;
	char			psem_name[MAXPATHLEN];
};

struct _psem_fdinfo {
	struct _proc_fileinfo	pfi;
	struct _psem_info	pseminfo;
};



struct _pshm_info  {
	struct _vinfo_stat	pshm_stat;
...
struct _pshm_fdinfo {
	struct _proc_fileinfo	pfi;
	struct _pshm_info	pshminfo;
};


struct _pipe_info {
	struct _vinfo_stat	pipe_stat;
...
struct _pipe_fdinfo {
	struct _proc_fileinfo	pfi;
	struct _pipe_info	pipeinfo;
};


struct _kqueue_info {
	struct _vinfo_stat	kq_stat;
	uint32_t		kq_state;
	uint32_t		rfu_1;	/* reserved */
};
...
struct _kqueue_fdinfo {
	struct _proc_fileinfo	pfi;
	struct _kqueue_info	kqueueinfo;
};

struct _appletalk_info {
	struct _vinfo_stat	atalk_stat;
};

struct _appletalk_fdinfo {
	struct _proc_fileinfo	pfi;
	struct _appletalk_info	appletalkinfo;
};
...
#define PROC_PIDWORKQUEUEINFO_SIZE	(sizeof(struct _proc_workqueueinfo))
